...
| **D-PRET-0** ✅ | **LE LOADER NE FAIT PAS DE PRETS. Point.** Tranche le 12/08/2026 par **Yaniv, auteur du CDC**. Ce qui est repris de Duhamel est sa **METHODOLOGIE**, jamais son objet : les quatre profils comportementaux, l'ajustement contextuel a neuf variables, la compression temporelle, la notion de DPD. Le Loader n'emet **aucun** `CREATE_LOAN`, `REPAY_LOAN` ni `SET_DPD`. Il ne cree **aucun** produit de credit, qui ne servirait a rien. Concordance mesuree : CDC ligne 326 — « neuf services de production confirmes… les modules bulk-payment, **loan, lender** et notification sont a l'etat de specification » ; ligne 375 — « injection dans les **neuf** services vivants » ; `loan-service/health` et `lending-service/health` rendent **000** (injoignables, mesure du 12/08). `UC-02`, `UC-03`, `EF-70` et `EF-71` decrivent une capacite FUTURE, hors du perimetre livrable. |
| **A-02** ✅ | **`EF-80` inapplicable tel qu'écrit — recommandation APPLIQUÉE le 12/08.** Le `segment` émis à l'onboarding vient désormais de la **même strate** que `solde_initial()` : les onze signaux `quick_win` que la famille A porte réellement, projetés sur les cinq valeurs de l'Annexe E. Rien de neuf n'est inventé — c'est le même signal mesuré, exprimé sur l'axe que le serveur offre. Mesure : distribution en cloche (1,8 / 24 / 50,4 / 22,8 / 1,0 %) et monotonie stricte segment↔solde. Aucun conflit produit possible : les 8 produits du serveur portent `segment: ANY`. |
| **A-02** *(énoncé d'origine)* | **`EF-80` inapplicable tel qu'écrit** | Les champs `decision.*` n'existent pas, et les 2000 clients viennent de la famille A qui ne porte aucune décision. Deux options : le Loader attribue lui-même APPROVED/DECLINED dans les proportions du CDC (l'isolation vis-à-vis de ReadyScore reste totale), ou seuls quelques clients ont un vrai scoring. **Recommandation : la première.** |
